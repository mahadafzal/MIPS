...
// Initializes all the fields for every instruction in Opcode enum
{
  ...
  myArray[BEQ].name = "beq";  myArray[BEQ].numOps = 3;  myArray[BEQ].rdPos = -1;  myArray[BEQ].rsPos = 0;  myArray[BEQ].rtPos = 1;  myArray[BEQ].immPos = 2;  myArray[BEQ].instType = ITYPE;  myArray[BEQ].op_field = "000100"; myArray[BEQ].funct_field = "";
}

Opcode OpcodeTable::getOpcode(string opc)
// Given a string representation of a valid 6 bit opfield, returns an Opcode which represents a
...
Opcode OpcodeTable::getOpcode(string opc, string funct)
// Given a string representation of a valid 6 bit opfield and a 6 bit function field, returns an Opcode which 
// represents a template for that instruction. Only utilizes the function field if it is R-Type instruction.
{
  for(int i = 0; i <(int)UNDEFINED; i++){
    if(myArray[i].op_field == opc){
      // checks if op_field corresponds to that of an R-type, and then uses func_field to get Opcode
      if(myArray[i].op_field == "000000"){
        for(int j = 0; i <(int)UNDEFINED; j++){
          if(myArray[j].funct_field == funct){
            return (Opcode)j;
          }
        }
      }
...
// Given an Opcode, returns the name of the instruction. 
{
  if(o < 0 || o >= UNDEFINED)
    return "";
